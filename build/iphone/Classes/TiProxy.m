...
//Rare exceptions to indicate a bug in the _cotm code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_cotm = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_cotm]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.cotm.module"
